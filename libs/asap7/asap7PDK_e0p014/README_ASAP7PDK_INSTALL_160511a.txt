...
▾ cdslib/						: Directory containing the ASAP7 technology file and technology
  ...
  ▸ asap7_TechLib_06/					: ASAP7 technology library.
  ...
  ▾ setup/						: Contains the setup files for Cadence Virtuoso.
      ...
      cshrc_asap7_eecad_v01				: A sample file containing the environmental variables required
							  to enable the use of various Cadence, Mentor Graphics, and
							  Synopsys tools. The paths are specific to the eecad computers.
...
▸ asap7ssc7p5t_02					: A design library containing sample standard cells. Defined as 
...
1. Untar the PDK archive 'asap7PDK_e0p014.tar.bz2' (where 0p014 is the PDK
   version number) by using the following command:

   tar -xvjf asap7PDK_e0p014.tar.bz2 

2. Copy the untarred PDK 'asap7PDK_e0p014' to an install area.
...
3. Change the environment variable value $PDK_DIR in the following files by
   ...
   directory (including the PDK directory name):
	...
	./cdslib/setup/cshrc_asap7_eecad_v01
...
3. Go to the run directory created in the above step. Then use the
   ...
   .cdsinit, .cdsenv, and .cshrc file, required to use the PDK, into your
   run directory.
